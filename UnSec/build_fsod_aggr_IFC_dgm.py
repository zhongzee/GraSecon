...
# 这个UnSec_GT_by_level_IFC_gm2结果还行 没有怎么降低！这个是针对GT情况
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', default='./nexus/fsod/vitB32/UnSec_GT_by_level_TFC_dgm_1227')
    ...
    parser.add_argument('--sentence_type', default='by_level', choices=['by_level', 'candidate', 'detail', 'combined']) # 这个选了没用
    parser.add_argument('--enable_global_mean', action='store_true', default=True, help='是否开启多领域总体均值校正') # 对于FSOD需要开启，对于iNat则不需要开启
    ...
    # 遍历修正后的特征
    for level_name, level_ids in corrected_tree_features.items():  # 修改点
        total_num = len(list(level_ids.values()))
        print(f"Total feats = {total_num} at {level_name}")

    # 准备并保存修正后的特征
    for level_name, level_theme_dict in corrected_tree_features.items():  # 修改点
