...
if __name__ == '__main__':
    ...
    text_calculator = TextMeanFeatureCalculator2(save_dir='./nexus/fsod/vitB32/UnSec_GT_by_level_IFC', device=device)
    ...
    for level_name, level_data in theme_tree_features.items():
        for unique_id, text_feature in level_data.items():
            ...
            # 定义非叶子层级，根据您的数据集结构调整
            non_leaf_levels = ['l1', 'l2']  # 示例，假设 'l1' 和 'l2' 是非叶子层级
  
            # 正确：基于层级名称决定如何修正特征
            for level_name, level_data in theme_tree_features.items():
                for unique_id, text_features in level_data.items():
                    if level_name in non_leaf_levels:
                        # 非叶子层级，确保是列表
                        if not isinstance(text_features, list):
                            logger.error(f"Expected list for {level_name}_{unique_id}, got {type(text_features)}")
                            continue  # 跳过或根据需要处理
                        for idx, feature in enumerate(text_features):
                            corrected = correct_domain_bias_fsod(
                                text_features=feature,
                                domain_mean=mean_features[level_name],
                                cross_level_mean=cross_level_mean if args.enable_cross_level_mean else None,
                                global_mean=global_mean_features if args.enable_global_mean else None,
                                parent_mean=parent_mean,
                                child_mean=child_mean,
                                level_name=level_name,
                                delta=0.1
                            )
                            logger.debug(f"Before correction: {feature[:5]}...")
                            logger.debug(f"After correction: {corrected[:5]}...")
                            theme_tree_features[level_name][unique_id][idx] = corrected
                        node_name = category_id_to_name[level_name].get(unique_id, 'Unknown')
                        logger.info(f"Applied domain bias correction for '{unique_id}' ('{node_name}') at level '{level_name}'")
                    else:
                        # 叶子层级，直接修正
                        if not isinstance(text_features, torch.Tensor):
                            logger.error(f"Expected Tensor for {level_name}_{unique_id}, got {type(text_features)}")
                            continue  # 跳过或根据需要处理
                        corrected = correct_domain_bias_fsod(
                        ...
                        logger.debug(f"Before correction: {text_features[:5]}...")
                        logger.debug(f"After correction: {corrected[:5]}...")
                        theme_tree_features[level_name][unique_id] = corrected
                        node_name = category_id_to_name[level_name].get(unique_id, 'Unknown')
                        logger.info(f"Applied domain bias correction for '{unique_id}' ('{node_name}') at level '{level_name}'")
    ...
    #         theme_tree_after_correction[level_name][level_cat_id] = theme_feat
    ...
    for level_name, level_ids in theme_tree_after_correction.items():
        total_num = len(list(level_ids.values()))
        print(f"Total feats = {total_num} at {level_name}")

        # Prepare and Save Features
    for level_name, level_theme_dict in theme_tree_after_correction.items():
