...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', default='./nexus/inat/vitB32/UnSec_inat_GT_detail_TFC_SR_0113')#UnSec_inat_GT_HSB_TFC_SR_0114
    ...
    for level_name, level_ids in tree_features.items():
        ...
        for level_cat_id, level_child_feats in level_ids.items():
            stacked_child_feats = torch.stack(level_child_feats)#torch.Size([340, 512])
            theme_feat = theme_maker.get_theme(stacked_child_feats) # torch.Size([512])

            # 获取当前节点的名称
            node_name = meta_tree[level_name]['categories'][int(level_cat_id) - 1]['name']
            node_name_feature = get_node_name_feature(node_name, global_encoder, device)  # torch.Size([512])

            print("当前优化层级：",level_name)
            print("当前优化节点：",node_name)
            # 调用优化函数
            optimized_feature = optimize_feature_with_adm_improved(
                current_feature=theme_feat,           # 使用语义增强后的 theme_feat
                node_name_feature=node_name_feature, # 逼近的目标 node_name_feature
                node_name=node_name,
                cat_id=level_cat_id,
                level_name=level_name,
                csv_writer=None,  # 如果需要，可以传递一个 CSV 写入器
                lambda_val=0.1,
                beta=0.001,
                rho=1.0,
            ...
            theme_tree_features[level_name][level_cat_id] = optimized_feature
