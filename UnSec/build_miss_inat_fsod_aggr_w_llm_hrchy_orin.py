    ...
    if args.dataset_name in ["inat_expanded", "fsod_expanded"]:
        ...
        for level_name in level_names[:1]:
            ...
            for cat_id, entry in sorted(merged_dict.items(), key=lambda item: int(item[0])):
                ...
                node_sentences = entry["detail_sentences"] if args.aggregator != "plain" else entry["node_name"]
                ...
                node_tokens = clip.tokenize(node_sentences).to(device) # CLIP只处理candidate_sentences
                with torch.no_grad():
