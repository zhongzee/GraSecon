...
def compute_global_mean_all_layers(tree_features):
    """
    计算所有层级的全局均值 μ_avg。
    
    Args:
        tree_features (dict): 层级特征字典。
...
def compute_global_mean_first_two_layers(tree_features, first_n_layers=2):
    """
    计算前几层级（例如前两层）的全局均值 μ_avg。
    
    Args:
        tree_features (dict): 层级特征字典。
        first_n_layers (int): 选择的前几层级。
    ...
    all_features = []
    # 假设层级名称按顺序为 'l1', 'l2', 'l3', ...
    sorted_layers = sorted(tree_features.keys(), key=lambda x: int(x[1:]))  # 例如 'l1', 'l2', ...
    selected_layers = sorted_layers[:first_n_layers]
    for layer in selected_layers:
        level_data = tree_features[layer]
        for feature in level_data.values():
            if isinstance(feature, list):
                all_features.extend(feature)
            else:
...
# 2. 层级特定的特征正交化
def layer_specific_feature_orthogonalization(tree_features, global_mean, layer='l3'):
    """
    对指定层级的特征进行正交化处理，确保其与全局均值正交。
    
    Args:
        tree_features (dict): 层级特征字典，结构为 {层级: {唯一ID: 特征向量或特征向量列表}}。
        global_mean (Tensor): 全局均值特征向量，形状为 [feature_dim]。
        layer (str): 需要进行正交化的层级名称。
    ...
    for unique_id, feature in tree_features[layer].items():
        if isinstance(feature, list):
            corrected_features = []
            for feat in feature:
                projection = torch.dot(feat, global_mean) / torch.dot(global_mean, global_mean) * global_mean
                orthogonal_feat = feat - projection
                orthogonal_feat = orthogonal_feat / torch.norm(orthogonal_feat, p=2)
                corrected_features.append(orthogonal_feat)
            corrected_tree_features[layer][unique_id] = corrected_features
        else:
            projection = torch.dot(feature, global_mean) / torch.dot(global_mean, global_mean) * global_mean
            orthogonal_feat = feature - projection
            orthogonal_feat = orthogonal_feat / torch.norm(orthogonal_feat, p=2)
            corrected_tree_features[layer][unique_id] = orthogonal_feat
    
    return corrected_tree_features

# 3. 层级特定的固定特征调整
def layer_specific_fixed_feature_adjustment(tree_features, transform_matrix, layer='l3'):
    """
    对指定层级的特征应用固定的变换（旋转或缩放）。
    
    Args:
        tree_features (dict): 层级特征字典，结构为 {层级: {唯一ID: 特征向量或特征向量列表}}。
        transform_matrix (Tensor): 固定的变换矩阵，形状为 [feature_dim, feature_dim]。
        layer (str): 需要进行特征调整的层级名称。
    ...
    for unique_id, feature in tree_features[layer].items():
        if isinstance(feature, list):
            corrected_features = []
            for feat in feature:
                transformed_feat = torch.matmul(transform_matrix, feat)
                transformed_feat = transformed_feat / torch.norm(transformed_feat, p=2)
                corrected_features.append(transformed_feat)
            corrected_tree_features[layer][unique_id] = corrected_features
        else:
            transformed_feat = torch.matmul(transform_matrix, feature)
            transformed_feat = transformed_feat / torch.norm(transformed_feat, p=2)
            corrected_tree_features[layer][unique_id] = transformed_feat
    
    return corrected_tree_features

# 4. 层级特定的标准化与正交化组合
def layer_specific_orthandstand(tree_features, mean_features, global_mean, layer='l3', gamma=1.0, beta=0.0, transform_matrix=None):
    """
    同时对指定层级的特征进行标准化和正交化处理。
    
    Args:
        tree_features (dict): 层级特征字典，结构为 {层级: {唯一ID: 特征向量或特征向量列表}}。
        mean_features (dict): 每个层级的均值特征，结构为 {层级: 均值向量}。
        global_mean (Tensor): 全局均值特征向量，形状为 [feature_dim]。
        layer (str): 需要处理的层级名称。
        gamma (float): 标准化的缩放因子。
        beta (float): 标准化的偏移因子。
        transform_matrix (Tensor, optional): 固定的变换矩阵，形状为 [feature_dim, feature_dim]。
    ...
    for unique_id, feature in tree_features[layer].items():
        if isinstance(feature, list):
            ...
            for feat in feature:
                # 标准化
                standardized_feat = (feat - mean_features[layer]) / (tree_features[layer][unique_id].std(dim=0) + 1e-5)
                standardized_feat = standardized_feat * gamma + beta
                standardized_feat = standardized_feat / torch.norm(standardized_feat, p=2)
                # 正交化
                projection = torch.dot(standardized_feat, global_mean) / torch.dot(global_mean, global_mean) * global_mean
                orthogonal_feat = standardized_feat - projection
                orthogonal_feat = orthogonal_feat / torch.norm(orthogonal_feat, p=2)
                # 应用固定变换
                if transform_matrix is not None:
                    orthogonal_feat = torch.matmul(transform_matrix, orthogonal_feat)
                    orthogonal_feat = orthogonal_feat / torch.norm(orthogonal_feat, p=2)
                corrected_features.append(orthogonal_feat)
            corrected_tree_features[layer][unique_id] = corrected_features
        else:
            # 标准化
            standardized_feat = (feature - mean_features[layer]) / (tree_features[layer][unique_id].std(dim=0) + 1e-5)
            standardized_feat = standardized_feat * gamma + beta
            standardized_feat = standardized_feat / torch.norm(standardized_feat, p=2)
            # 正交化
            projection = torch.dot(standardized_feat, global_mean) / torch.dot(global_mean, global_mean) * global_mean
...
# 5. 综合偏置校正函数
def apply_layer_specific_bias_correction_all(tree_features, mean_features, global_mean=None, layer_policy=None, no_gm_strategy='Standardization', transform_matrix=None):
    """
    对不同层级应用不同的偏置修正策略。
    ...
    Args:
        ...
        global_mean (Tensor, optional): 全局均值特征向量。
        layer_policy (dict, optional): 每个层级的策略，例如 {'l1': 'no_gm', 'l2': 'gm'}。
        no_gm_strategy (str, optional): 对于 'no_gm' 层级应用的策略，选项包括 'Standardization', 'Orthogonalization', 'mix', 'OrthandStand', 'all'。
        transform_matrix (Tensor, optional): 固定的变换矩阵，仅在 'mix' 或 'OrthandStand' 策略中使用。
    ...
    for level_name, level_data in tree_features.items():
        ...
        if policy == 'gm':
            ...
            elif strategy == 'orthogonalization':
                if global_mean is None:
                    raise ValueError("global_mean must be provided for orthogonalization strategy.")
                corrected_tree_features = layer_specific_feature_orthogonalization(corrected_tree_features, global_mean, layer=level_name)
            
            elif strategy == 'mix':
                if global_mean is None:
                    raise ValueError("global_mean must be provided for mix strategy.")
                if transform_matrix is None:
                    raise ValueError("transform_matrix must be provided for mix strategy.")
                # 首先进行正交化
                corrected_tree_features = layer_specific_feature_orthogonalization(corrected_tree_features, global_mean, layer=level_name)
                # 然后应用固定的特征调整
                corrected_tree_features = layer_specific_fixed_feature_adjustment(corrected_tree_features, transform_matrix, layer=level_name)
            
            elif strategy == 'orthandstand':
                if global_mean is None:
                    raise ValueError("global_mean must be provided for orthandstand strategy.")
                # 'orthandstand' 不应用固定变换，仅执行标准化和正交化
                mu, sigma = layer_stats[level_name]
                for unique_id, feature in level_data.items():
                    if isinstance(feature, list):
                        corrected_features = []
                        ...
                        orthogonal_feat = standardized_feat - projection
                        orthogonal_feat = orthogonal_feat / torch.norm(orthogonal_feat, p=2)
                        corrected_tree_features[level_name][unique_id] = orthogonal_feat
            
            elif strategy == 'all':
                # 应用所有策略：标准化、正交化、固定特征调整
                if global_mean is None:
                    raise ValueError("global_mean must be provided for all strategy.")
                if transform_matrix is None:
                    raise ValueError("transform_matrix must be provided for all strategy.")
                mu, sigma = layer_stats[level_name]
                for unique_id, feature in level_data.items():
                    if isinstance(feature, list):
                        corrected_features = []
                        for feat in feature:
                            # 标准化
                            standardized_feat = (feat - mu) / sigma
                            standardized_feat = standardized_feat * 1.0 + 0.0
                            standardized_feat = standardized_feat / torch.norm(standardized_feat, p=2)
                            # 正交化
                            projection = torch.dot(standardized_feat, global_mean) / torch.dot(global_mean, global_mean) * global_mean
                            orthogonal_feat = standardized_feat - projection
...
# 选择策略
no_gm_strategy = 'Orthogonalization'  # 可选 'Standardization', 'Orthogonalization', 'mix', 'OrthandStand', 'all'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', default='./nexus/fsod/vitB32/UnSec_llm_TFC_new_3dgm_Ortho') # UnSec_llm_by_level_TFC_dgm
   ...
   # 创建嵌套的类别名称到ID的映射
    ...
    # 选择使用所有层的 global_mean 或仅前两层的
    use_all_layers_mean = True  # 设置为 False 使用前两层的均值

    if use_all_layers_mean:
        selected_global_mean = compute_global_mean_all_layers(theme_tree_features)
    else:
        selected_global_mean = compute_global_mean_first_two_layers(theme_tree_features, first_n_layers=2)
    ...
    # 应用偏置校正
    corrected_tree_features = apply_layer_specific_bias_correction_all(theme_tree_features, mean_features, global_mean=selected_global_mean, layer_policy=layer_policy,no_gm_strategy=no_gm_strategy,transform_matrix=None)
