...
if __name__ == '__main__':
    ...
    if args.mode == 'query' and args.dataset_name == 'inat':
        ...
        results_dict = {cat_id: {'node_name': cat_name, 'parent_names': cat_parents, 'child_names': []}
                        for cat_id, cat_name, cat_parents in zip(node_ids, node_names, node_parents)} # 5个类
        # prompter在这里定义
        ...
        for cat_id, cat_entry in results_dict.items():
            ppt = h_prompter.embed(node_name=cat_entry['node_name'], context=level_contexts[args.h_level]) # Generate a list of 10 Class of the following Phylum and output the list separated by '&' (without numbers): Chordata
