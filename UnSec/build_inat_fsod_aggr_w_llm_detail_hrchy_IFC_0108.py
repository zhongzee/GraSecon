...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', default='./nexus/inat/vitB32/UnSec_by_level_IFC_0108') # UnSec_llm_IFC_0108
    ...
    text_calculator = TextMeanFeatureCalculator2(save_dir='./nexus/inat/vitB32/UnSec_llm_IFC_0108',device=device)
    ...
    mean_features = text_calculator.compute_mean_features(args.dataset_name, all_text_features)
    ...
    for level_name, level_data in theme_tree_features.items():
        for cat_id, text_feature in level_data.items():
            if level_name in mean_features:
                domain_mean = mean_features[level_name]  # 获取当前粒度的均值
                domain_invariant_mean = None
                if args.enable_global_mean:  # 如果开启多领域总体均值校正
                    domain_invariant_mean = torch.stack(list(mean_features.values())).mean(dim=0)  # 计算所有粒度的总体均值

                # 校正领域偏置
                corrected_feature = text_calculator.correct_domain_bias(
                    text_feature,
                    domain_mean,
                    domain_invariant_mean if args.enable_global_mean else None  # 如果未启用则传入 None
                )
                theme_tree_features[level_name][cat_id] = corrected_feature
