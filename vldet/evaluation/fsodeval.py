...
class FSODEval:
    def __init__(
        ...
        """Constructor for FSOD 200 evaluation.
...
def _evaluate_predictions_on_fsod(
        fsod_gt,                # ground-truth annotation file
        fsod_results_path,      # model prediction results saved from inference stage
        eval_seg=False,
    ...
    results = {}
    fsod_eval = FSODEval(fsod_gt, fsod_results_path, 'bbox', expand_pred_label=False)
    fsod_eval.run()
    fsod_eval.print_results()
    results["AP50"] = fsod_eval.get_results()["AP50"]

    mAP = np.zeros(len(class_names)) - 1

    precisions = fsod_eval.eval['precision']
    assert len(class_names) == precisions.shape[2]

    results_per_category = []
    id2apiid = sorted(fsod_gt.get_cat_ids())
    ...
    for idx, name in enumerate(class_names):
        ...
        inst_num = len(fsod_gt.get_ann_ids(cat_ids=[id2apiid[idx]]))
        if inst_num > 0:
...
class FSODEvaluator(DatasetEvaluator):
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        self._fsod_api = LVIS(json_file)
        # Test set json files do not contain annotations (evaluation must be
        # performed using the LVIS evaluation server).
        self._do_evaluation = len(self._fsod_api.get_ann_ids()) > 0
    ...
    def evaluate(self):
        ...
        self._logger.info("Preparing results in the FSOD 500 format ...")
        self._fsod_results = list(
            itertools.chain(*[x["instances"] for x in self._predictions]))

        # unmap the category ids for LVIS (from 0-indexed to 1-indexed)
        for result in self._fsod_results:
            result["category_id"] += 1

        PathManager.mkdirs(self._output_dir)
        file_path = os.path.join(
            self._output_dir, "fsod_instances_results.json")
        self._logger.info("Saving results to {}".format(file_path))
        with PathManager.open(file_path, "w") as f:
            f.write(json.dumps(self._fsod_results))
            f.flush()
        ...
        res, mAP = _evaluate_predictions_on_fsod(
            self._fsod_api,
            file_path,
        ...
        mAP_out_path = os.path.join(self._output_dir, "fsod_mAP.npy")
