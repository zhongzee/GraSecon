...
def do_GraSecon(args, aggr_method, model, dloader, class_tree, template='a {}'):
    ...
    for cat_id, entry in tqdm(sorted(class_tree.items(), key=lambda item: int(item[0]))):
        ...
        zeroshot_weights.append(node_name_feature)
    # zeroshot_weights.append(txt_embeddings)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--hierarchy_tree_path',
                      type=str,
                      default="./GraSecon_cls/hrchy_imagenet1k/imagenet1k_detail_llm_composed.json",
                      choices=[
                          ...
                          "./GraSecon_cls/hrchy_imagenet1k/imagenet1k_detail_llm_composed.json",  # 修改了这里
                      ],
  )
  ...
  parser.add_argument('--out_path', default='./nexus/imagenet2012/vitB32/GraSecon_llm_by_level_TFC_0113_new_layer_policy_5n1g_new_epoch2_dgm') # GraSecon_llm_by_level_TFC_0109_new_layer_policy_5n1g,GraSecon_llm_by_level_TFC_0109_new_layer_policy_6g，GraSecon_llm_by_level_TFC_0111_new_layer_policy_5n1g_w_SR_epoch100
  ...
  if args.method == "zeroshot":
    ...
    for i in range(num_runs):
      GraSecon_mean_results = do_GraSecon(args, "mean", encoder, dloader, class_tree, template='a {}')
      total_fps += GraSecon_mean_results[-1]

    GraSecon_mean_results = list(GraSecon_mean_results)
    GraSecon_mean_results[-1] = total_fps / num_runs
    print_results(method=f"GraSecon-Mean_w_{args.model_size}", results=GraSecon_mean_results,
                  hierarchy_source=args.hierarchy_tree_path)
  else:
