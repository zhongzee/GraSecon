...
class MeanFeatureCalculator2:
    ...
    def compute_mean_features_imagenet(self, dataset_name, data_loader, model,
        ...
        直接使用 CLIP 的 encode_image 方法提取图像特征，并计算均值特征。
        """

        script_path = "../scripts_local/Detic/imagenet2012/GraSecon_cls/zeroshot.sh"
        ...
        # 定义均值特征文件路径
        mean_feature_path = os.path.join(dataset_save_dir, f"{dataset_name}_mean_features.npy")

        # 如果均值特征文件已经存在，直接加载并返回
        if os.path.exists(mean_feature_path):
            # self.logger.info(f"均值特征文件已存在，直接加载 {mean_feature_path}")
        ...
        # 使用 tqdm 显示进度条
        for idx, (images, labels) in enumerate(tqdm(data_loader, desc="Encoding images")):
            # 将图像移动到指定设备
        ...
        self.logger.info(f"均值特征已保存至 {mean_feature_path}")
    ...
    # def _correct_domain_bias_imagenet2012(self, img_embeddings, mean_features):
    #     """
    #     纠正领域偏置：减去对应的领域均值特征，并进行标准化。

    #     Args:
    #         img_embeddings (Tensor): Backbone 输出的特征张量，形状为 [B, C]。
    #         mean_features (ndarray): 领域均值特征数组，形状为 [num_classes, C]，每一行对应一个类别的均值特征。

    #     Returns:
    #         corrected_img_embeddings (Tensor): 校正后的特征张量，形状为 [B, C]。
    #     """
    #     corrected_img_embeddings = []

    #     for cat_id in range(mean_features.shape[0]):
    #         mean_tensor = torch.tensor(mean_features[cat_id]).to(img_embeddings.device).view(1, -1)  # 转换为张量并调整维度
    #         centered_embeddings = img_embeddings - mean_tensor  # 减去领域均值特征

    #         # 计算 L2 范数（按通道归一化）
    #         norm = torch.norm(centered_embeddings, p=2, dim=1, keepdim=True)  # 按特征维度计算范数
    #         corrected_embedding = centered_embeddings / (norm + 1e-6)  # 归一化

    #         corrected_img_embeddings.append(corrected_embedding)

    #     corrected_img_embeddings = torch.stack(corrected_img_embeddings)  # 合并所有校正后的特征

    #     return corrected_img_embeddings

    def _correct_domain_bias_imagenet2012(self, img_embeddings, mean_features):
        """
        纠正领域偏置：减去全局均值特征，并进行标准化。

        Args:
            img_embeddings (Tensor): Backbone 输出的特征张量，形状为 [B, C]。
            mean_features (Tensor): 全局均值特征，形状为 [C]。

        Returns:
            corrected_img_embeddings (Tensor): 校正后的特征张量，形状为 [B, C]。
        """
        # 将 mean_features 转为张量并调整到相同的设备
        mean_tensor = torch.tensor(mean_features).to(img_embeddings.device).view(1, -1)  # 转换为 [1, C]

        # 减去全局均值特征
        centered_embeddings = img_embeddings - mean_tensor  # [B, C] - [1, C] = [B, C] 

        # 计算 L2 范数（按通道归一化）
        norm = torch.norm(centered_embeddings, p=2, dim=1, keepdim=True)  # [B, 1]
        corrected_embedding = centered_embeddings / (norm + 1e-6)  # 归一化
