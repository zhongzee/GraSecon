...
class MeanFeatureCalculator:
    ...
    def compute_mean_features(self, cfg, dataset_name, data_loader, model,
                         max_samples=None):
        """
        ...
        weights_base, _ = os.path.splitext(weights_filename)  # 去除扩展名
        ...
        # 构建 script_path
        script_path = os.path.join(script_prefix, weights_base)
        # 如果需要添加 .sh 扩展名，可以取消下面注释
        # script_path = os.path.join(script_prefix, weights_base + ".sh")
        ...
        if all(os.path.exists(path) for path in mean_feature_paths.values()):
            self.logger.info("均值特征文件已存在，直接加载...")
            for level, path in mean_feature_paths.items():
                # mean_features[level] = np.load(path)
                mean_features[level] = np.load(path, allow_pickle=True)
...
def visualize_features(backbone_features, corrected_backbone_features, dataset_name, idx):
    """
    Visualize the feature maps from the backbone and the corrected feature maps.
    
    Args:
        backbone_features (dict): Original backbone feature maps (p3, p4, p5, etc.).
        corrected_backbone_features (dict): Corrected backbone feature maps (p3, p4, p5, etc.).
        dataset_name (str): The dataset name to be included in the image filename.
        idx (int): The index to differentiate the saved image file.
    """
    # Define the layers to visualize
    # layers = ['p3', 'p4', 'p5', 'p6','p7']
    layers = ['p7']
    # Iterate over each layer and visualize the features
    for layer in layers:
        # Create the plot for original and corrected feature maps
        fig, axes = plt.subplots(1, 2, figsize=(12, 6))

        # Plot the original feature map
        axes[0].imshow(backbone_features[layer].squeeze(0).cpu().numpy()[0], cmap='YlGnBu')  # Use a colormap
        axes[0].set_title(f"Original {layer}", fontsize=16)
        axes[0].axis('off')

        # Plot the corrected feature map
        axes[1].imshow(corrected_backbone_features[layer].squeeze(0).cpu().numpy()[0], cmap='YlGnBu')
        axes[1].set_title(f"Corrected {layer}", fontsize=16)
        axes[1].axis('off')

        # Prepare the save path, ensuring the base path exists
        base_path = f"./visualize/image_visualize/{dataset_name}/{layer}"
        os.makedirs(base_path, exist_ok=True)  # Ensure the path exists, create if necessary

        # Define the save path with dataset name, layer, and index
        save_path = os.path.join(base_path, f"{dataset_name}_{layer}_{idx}.png")
        print(f"Saving to: {save_path}")  # Print the save path for debugging
...
def visualize_features_map(backbone_features, corrected_backbone_features, dataset_name, idx,zs_weight_path, device):
    """
    可视化图像特征图（校正前和校正后的对比），并与zs_weight相乘，计算相似度可视化。
    
    Args:
        backbone_features (dict): 原始 backbone 特征图（p3, p4, p5, 等）。
        corrected_backbone_features (dict): 校正后的 backbone 特征图。
        dataset_name (str): 数据集名称。
        idx (int): 用于区分文件的索引。
        zs_weight_path (str): zs_weight路径，包含分类权重。
        device (str): 设备（cuda或cpu）。
    """
    # 加载zs_weight
    zs_weight = torch.tensor(np.load(zs_weight_path), dtype=torch.float32).to(device)  # 加载npy文件
    zs_weight = zs_weight.permute(1, 0).contiguous()  # 转置为 [D, C]

    layers = ['p3', 'p4', 'p5', 'p6']
    for layer in layers:
        # 获取原始特征和校正后的特征图
        original_feat = backbone_features[layer].squeeze(0).cpu().numpy()
        corrected_feat = corrected_backbone_features[layer].squeeze(0).cpu().numpy()

        # 计算相似度，原始特征与zs_weight计算
        similarity_original = np.dot(original_feat.flatten(), zs_weight.flatten())  # 计算相似度
        similarity_corrected = np.dot(corrected_feat.flatten(), zs_weight.flatten())  # 计算校正后相似度

        # 创建图表进行原始特征图和校正后特征图的对比
        fig, axes = plt.subplots(1, 3, figsize=(18, 6))

        # 可视化原始特征图
        axes[0].imshow(original_feat[0], cmap='coolwarm')
        axes[0].set_title(f"Original {layer} Similarity: {similarity_original:.4f}", fontsize=16)
        axes[0].axis('off')

        # 可视化校正后的特征图
        axes[1].imshow(corrected_feat[0], cmap='coolwarm')
        axes[1].set_title(f"Corrected {layer} Similarity: {similarity_corrected:.4f}", fontsize=16)
        axes[1].axis('off')

        # 可视化相似度热图
        similarity_map = np.zeros_like(original_feat[0])  # 创建一个相似度图
        similarity_map.fill(similarity_corrected)  # 将校正后相似度值填充进图中
        axes[2].imshow(similarity_map, cmap='hot')
        axes[2].set_title(f"Similarity Map {layer}", fontsize=16)
        axes[2].axis('off')

        # 保存图像
        save_path = f"./visualize/image_visualize/{dataset_name}/{dataset_name}_{layer}_{idx}.png"
        print(f"Saving to: {save_path}")  # 打印保存路径
        plt.tight_layout()
        plt.savefig(save_path, dpi=600)
        plt.close()
