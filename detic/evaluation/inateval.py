...
# Part of the code is from https://github.com/facebookresearch/Detic/blob/main/detic/evaluation/oideval.py
...
class INATEval:
    def __init__(
            self, lvis_gt, lvis_dt, iou_type="bbox",
            expand_pred_label=False,
            oid_hierarchy_path='./datasets/oid/annotations/challenge-2019-label500-hierarchy.json'):
        """Constructor for iNatLoc500 evaluation.
        This is where the custom evaluation protocol is done!
        Args:
            lvis_gt: ground-truth annotation (LVIS class instance, or str containing path of annotation file)
            lvis_dt: prediction results (LVISResult class instance, or str containing path of result file,
                     or list of dict)
    ...
    def _prepare(self):
        ...
        for ann in gts:
            ...
            # img_pl = \n{img_pl}
            # img_pl[ann["image_id"]] = {img_pl[ann["image_id"]]}
            # """)
...
def _evaluate_predictions_on_inat(
        inat_gt,                # ground-truth annotation file
        inat_results_path,      # model prediction results saved from inference stage
        eval_seg=False,
        class_names=None,       # category names
):
    """
    Returns:
        results: a dict that has keys "AP50", "AP50_segm", "AP50_expand", etc.
        mAP: a list of float, in which the item is per-category AP
    """
    logger = logging.getLogger(__name__)
    metrics = ["AP50", "AP50_expand"]

    # >>> actualy evaluation process
    results = {}
    inat_eval = INATEval(inat_gt, inat_results_path, 'bbox', expand_pred_label=False)
    inat_eval.run()
    inat_eval.print_results()
    results["AP50"] = inat_eval.get_results()["AP50"]

    mAP = np.zeros(len(class_names)) - 1

    precisions = inat_eval.eval['precision']
    assert len(class_names) == precisions.shape[2]

    results_per_category = []
    id2apiid = sorted(inat_gt.get_cat_ids())
    ...
    for idx, name in enumerate(class_names):
        ...
        inst_num = len(inat_gt.get_ann_ids(cat_ids=[id2apiid[idx]]))
        if inst_num > 0:
            results_per_category.append((
                # cname num_of_instances
                "{} {}".format(
                    name.replace(' ', '_'),
...
class INATEvaluator(DatasetEvaluator):
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        self._inat_api = LVIS(json_file)
        # Test set json files do not contain annotations (evaluation must be
        # performed using the LVIS evaluation server).
        self._do_evaluation = len(self._inat_api.get_ann_ids()) > 0
    ...
    def evaluate(self):
        ...
        self._logger.info("Preparing results in the iNatLoc500 format ...")
        self._inat_results = list(
            itertools.chain(*[x["instances"] for x in self._predictions]))

        # unmap the category ids for LVIS (from 0-indexed to 1-indexed)
        for result in self._inat_results:
            result["category_id"] += 1

        PathManager.mkdirs(self._output_dir)
        file_path = os.path.join(
            self._output_dir, "inat_instances_results.json")
        self._logger.info("Saving results to {}".format(file_path))
        with PathManager.open(file_path, "w") as f:
            f.write(json.dumps(self._inat_results))
            f.flush()
        ...
        # Miu: actual evaluation function
        res, mAP = _evaluate_predictions_on_inat(
            self._inat_api,
            file_path,
        ...
        # 分解原始路径
        # parts = self._output_dir.split('/')
        #
        # # 替换 '/root/' 为 './'
        # # 这里假设路径始终以 '/root/' 开头
        # if parts[1] == "root":
        #     parts[1] = "root/autodl-tmp"
        #
        # # 重新组合路径
        # new_path = "/" + os.path.join(*parts)
        print("self._output_dir=",self._output_dir)
        mAP_out_path = os.path.join(self._output_dir, "inat_mAP.npy")
