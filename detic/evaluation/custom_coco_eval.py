...
# Part of the code is from https://github.com/facebookresearch/Detic/blob/main/detic/evaluation/custom_coco_eval.py
...
# from ..data.miu_datasets.coco_zeroshot import categories_seen, categories_unseen
import pandas as pd
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        if save_path:
            with open(save_path + ".txt", "w") as f:
                f.write("Per-category {} AP:\n".format(iou_type))
                f.write(tabulate(results_per_category, headers=["Category", "AP"], tablefmt="plain") + "\n\n")
                f.write("Per-category {} AP50:\n".format(iou_type))
                f.write(tabulate(results_per_category50, headers=["Category", "AP50"], tablefmt="plain") + "\n\n")
                f.write("Seen {} AP50: {}\n".format(iou_type, results["AP50-seen"]))
                f.write("Unseen {} AP50: {}\n".format(iou_type, results["AP50-unseen"]))

        # Save results to CSV file using pandas
        if save_path:
            df = pd.DataFrame(results_per_category, columns=["Category", "AP"])
