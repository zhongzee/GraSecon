...
def reset_cls_test(model, cls_path, num_classes):
    ...
    if type(cls_path) == str:
        ...
        loaded_np = np.load(cls_path)
        print('Loaded cls_path shape:', loaded_np.shape)
        if loaded_np.ndim != 2:
            raise ValueError(f"Expected cls_path to be 2D, but got {loaded_np.ndim}D")
    ...
    zs_weight = torch.cat(
        ...
        dim=1) # D x (C + 1) # torch.Size([512, 6])
    if model.roi_heads.box_predictor[0].cls_score.norm_weight: # 这里是detic检测头
    ...
    for k in range(len(model.roi_heads.box_predictor)):
        ...
        model.roi_heads.box_predictor[k].cls_score.zs_weight = zs_weight # 在這裏修改預測分數?！！！！  [512,6] # 5 个 + 1 个背景類別。

def reset_cls_test_orin(model, cls_path, num_classes):
    model.roi_heads.num_classes = num_classes
    if type(cls_path) == str:
        print('Resetting zs_weight', cls_path)
        zs_weight = torch.tensor(
            np.load(cls_path),#  在這裏加載npy
            dtype=torch.float32).permute(1, 0).contiguous() # D x C
    else:
        zs_weight = cls_path
    zs_weight = torch.cat(
...
def reset_cls_test_coco(model, cls_path, num_classes):
    model.roi_heads.num_classes = num_classes
    if type(cls_path) == str:
        print('Resetting zs_weight', cls_path)
        zs_weight = torch.tensor(
