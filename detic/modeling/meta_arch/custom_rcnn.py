...
from visualize.visualize import VisualizationEvaluator
...
class CustomRCNN(GeneralizedRCNN):
    ...
    def visualize_results(self, batched_inputs, results):
        """
        将推理结果进行可视化。
        """
        visualizer = VisualizationEvaluator(save_dir="./visualize/iNat_visualize_results2",
                                            score_threshold=0.05)
        visualizer.process(batched_inputs, results)

    def check_bbox_rescaled_to_original(self,batched_inputs, processed_results, tolerance=5):
        """
        检查处理后的边界框是否已调整为原图尺寸。

        参数:
            batched_inputs (list of dict): 原始输入数据，包含图像尺寸信息。
            processed_results (list of dict): 后处理后的预测结果，包含预测的边界框信息。
            tolerance (int): 容忍像素差异，用于检查尺寸是否接近原图尺寸。

        返回:
            bool: 如果所有图像的边界框都已调整为原图尺寸则返回 True，否则返回 False。
        """
        all_rescaled_correctly = True  # 用于标识是否所有边界框均已被正确调整

        for input_data, result in zip(batched_inputs, processed_results):
            # 获取原图尺寸
            original_height, original_width = input_data['height'], input_data['width']

            # 使用 detector_postprocess 后的 image_sizes 获取处理后图像尺寸
            processed_height, processed_width = result["instances"].image_size

            # 检查尺寸是否一致，允许小范围的误差
            height_diff = abs(original_height - processed_height)
            width_diff = abs(original_width - processed_width)

            if height_diff > tolerance or width_diff > tolerance:
                print(f"Warning: Image {input_data['file_name']} dimensions mismatch. "
                      f"Original (HxW): {original_height}x{original_width}, "
                      f"Processed (HxW): {processed_height}x{processed_width}")
                all_rescaled_correctly = False

            # 打印边界框坐标以检查是否正确
            print(f"Processed bounding boxes for {input_data['file_name']}:")
            for bbox in result["instances"].pred_boxes.tensor.cpu().numpy():
                print(bbox)

        return all_rescaled_correctly
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            # all_correct = self.check_bbox_rescaled_to_original(batched_inputs, processed_results)
            # if all_correct:
            #     print("All bounding boxes are correctly rescaled to the original image size.")
            # else:
            #     print("Some bounding boxes may not be correctly rescaled.")
            #
            # self.visualize_results(batched_inputs, processed_results)
